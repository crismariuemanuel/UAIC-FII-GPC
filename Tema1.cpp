...
public:
	...
	int isIn(CComplex& x, int& iteratornuantarosu)
	{
		int rez = 0;
		CComplex z0, z1;
		z0 = CComplex(0, 0);
		for (iteratornuantarosu = 1; iteratornuantarosu < m.nriter; iteratornuantarosu++)
		{
			z1 = z0 * z0 + x;
	...
	void display(double xmin, double ymin, double xmax, double ymax)
	{
		...
		glBegin(GL_POINTS);
		int iteratornuanterosu;
		for (double x = xmin; x <= xmax; x += RX_JF)
			for (double y = ymin; y <= ymax; y += RY_JF)
			{
				iteratornuanterosu = 0;
				CComplex z(x, y);
				int r = isIn(z, iteratornuanterosu);
				//        z.print(stdout);
				if (r == 0)
				{
					...
					glColor3f(0.033 * iteratornuanterosu, 0.1, 0.1); // nuante de rosu
				}
...
public:
	void patrat(double lungime, int nivel, CPunct& p) {
		...
		if (nivel == 0) {
			...
			patrat(lungime / ratia, nivel - 1, punct1);
			patrat(lungime / ratia, nivel - 1, punct2);
			patrat(lungime / ratia, nivel - 1, punct3);
			patrat(lungime / ratia, nivel - 1, punct4);
			patrat(lungime / ratia, nivel - 1, punct5);
			patrat(lungime / ratia, nivel - 1, punct6);
			patrat(lungime / ratia, nivel - 1, punct7);
			patrat(lungime / ratia, nivel - 1, punct8);
			patrat(lungime, nivel - 1, punct9);
		}
	}
